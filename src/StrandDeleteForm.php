...
class StrandDeleteForm extends ConfirmFormBase {
  protected $id;
  protected $name;
  
  function getFormId() {
    return 'strand_delete';
  }

  function getQuestion() {
    return t('Are you sure you want to delete Strand ' . $this->name . '?'); 
  }
  ...
  function submitForm(array &$form, FormStateInterface $form_state) {
    try
    {
        strandStorage::delete($this->id);
        drupal_set_message(t('Strand: ' . $this->name . ' has been deleted.'  ));
    }
    catch(\Exception $e) {
        drupal_set_message(t("Sorry, that did not work. Please ensure Strand: " . $this->name . " is not being used in the definition of a Unit."), 'error');
        //return;
    }
